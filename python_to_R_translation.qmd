...
# histogram bins using FD rule
y_fd_all = hist(x, breaks = "FD", plot=F)
y_fd_clean = hist(xClean, breaks = "FD", plot=F)

# creating dataframe
convert_to_DF = function(y){
    len_ = length(y$breaks)
    df_ = tibble(
        data_index = (y$breaks[1:(len_-1)] + y$breaks[2:len_])/2, # take the mean of the breaksa
        data_value = y$counts
    )
    return (df_)
}
