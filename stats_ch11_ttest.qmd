...
## Python version:
```{python}
#| label: "Exercise 11 (python-version)"
# params
...
# z-score (written out for clarity)
dataz = data %>% 
    mutate(across(cols2zscore, ~scale(.x)[, 1], .names = "{.col}"))
...
```{python}
#| label: "Exercise 13 (python-version)"
